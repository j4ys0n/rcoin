/*!
 * chain.js - blockchain management for rcoin
...
const AsyncEmitter = require('revent');
...
class Chain extends AsyncEmitter {
  ...
  async getDeployments(time, prev) {
    ...
    // For some reason ravencoind has p2sh in the
    ...
    if (this.options.bip148 && this.network === Network.main) {
      if (witness !== thresholdStates.LOCKED_IN
        ...
        // since rcoin only operates on the sane
  ...
  /**
   * Find a locator. Analagous to ravencoind's `FindForkInGlobalIndex()`.
