...
const hash160 = require('rcrypto/lib/hash160');
const hash256 = require('rcrypto/lib/hash256');
const {safeEqual} = require('rcrypto/lib/safe');
const secp256k1 = require('rcrypto/lib/secp256k1');
...
class RPC extends RPCBase {
  ...
  async getInfo(args, help) {
    ...
    return {
      ...
      keypoolsize: 0,
      unlocked_until: 0,
      paytxfee: Amount.rvn(this.network.feeRate, true),
      relayfee: Amount.rvn(this.network.minRelay, true),
      errors: ''
    };
  }
  ...
  async getMempoolInfo(args, help) {
    ...
    return {
      ...
      mempoolminfee: Amount.rvn(this.mempool.options.minRelay, true)
    };
  }
  ...
  async getTXOut(args, help) {
    ...
    return {
      ...
      value: Amount.rvn(coin.value, true),
  ...
  async getTXOutSetInfo(args, help) {
    ...
    return {
      ...
      bytes_serialized: 0,
      hash_serialized: 0,
      total_amount: Amount.rvn(this.chain.db.state.value, true)
    };
  }
    ...
    if (!this.network.selfConnect) {
      if (this.pool.peers.size() === 0) {
        throw new RPCError(errs.CLIENT_NOT_CONNECTED,
          'Ravencoin is not connected!');
      }

      if (!this.chain.synced) {
        throw new RPCError(errs.CLIENT_IN_INITIAL_DOWNLOAD,
          'Ravencoin is downloading blocks...');
      }
    }
  ...
  async validateAddress(args, help) {
    if (help || args.length !== 1)
      throw new RPCError(errs.MISC_ERROR, 'validateaddress "ravencoinaddress"');
  ...
  async verifyMessage(args, help) {
    if (help || args.length !== 3) {
      throw new RPCError(errs.MISC_ERROR,
        'verifymessage "ravencoinaddress" "signature" "message"');
    }
  ...
  txToJSON(tx, entry) {
    ...
    for (let i = 0; i < tx.outputs.length; i++) {
      ...
      vout.push({
        value: Amount.rvn(output.value, true),
        n: i,
  ...
  entryToJSON(entry) {
    return {
      size: entry.size,
      fee: Amount.rvn(entry.deltaFee, true),
      modifiedfee: 0,
      time: entry.time,
      height: entry.height,
