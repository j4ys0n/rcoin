/**
 * uri.js - ravencoin uri parsing for rcoin
...
/**
 * URI
 * Represents a ravencoin URI.
 * @alias module:rvn.URI
...
class URI {
  ...
  fromString(str, network) {
    ...
    assert(str.length > 8, 'Not a ravencoin URI.');

    const prefix = str.substring(0, 8);

    assert(prefix === 'ravencoin:', 'Not a ravencoin URI.');
  ...
  toString() {
    ...
    if (this.amount !== -1)
      query.push(`amount=${Amount.rvn(this.amount)}`);
