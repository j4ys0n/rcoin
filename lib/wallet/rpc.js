/*!
 * rpc.js - ravencoind-compatible json rpc for rcoin.
...
const RPCBase = rweb.RPC;
const RPCError = rweb.RPCError;
...
const errs = {
  ...
  INVALID_REQUEST: rweb.errors.INVALID_REQUEST,
  METHOD_NOT_FOUND: rweb.errors.METHOD_NOT_FOUND,
  INVALID_PARAMS: rweb.errors.INVALID_PARAMS,
  INTERNAL_ERROR: rweb.errors.INTERNAL_ERROR,
  PARSE_ERROR: rweb.errors.PARSE_ERROR,
...
const MAGIC_STRING = 'Ravencoin Signed Message:\n';
...
class RPC extends RPCBase {
  ...
  async addMultisigAddress(args, help) {
    ...
    // Impossible to implement in rcoin (no address book).
  ...
  async dumpPrivKey(args, help) {
    if (help || args.length !== 1)
      throw new RPCError(errs.MISC_ERROR, 'dumpprivkey "ravencoinaddress"');
  ...
  async dumpWallet(args, help) {
    ...
    const out = [
      format('# Wallet Dump created by Rcoin %s', pkg.version),
  ...
  async getAccount(args, help) {
    if (help || args.length !== 1)
      throw new RPCError(errs.MISC_ERROR, 'getaccount "ravencoinaddress"');
  ...
  async getReceivedByAccount(args, help) {
    ...
    return Amount.rvn(total, true);
  }

  async getReceivedByAddress(args, help) {
    if (help || args.length < 1 || args.length > 2) {
      throw new RPCError(errs.MISC_ERROR,
        'getreceivedbyaddress "ravencoinaddress" ( minconf )');
    }
  ...
  async _toWalletTX(wtx) {
    ...
    for (let i = 0; i < details.outputs.length; i++) {
      ...
      if (member.path) {
        ...
        det.push({
          ...
          category: 'receive',
          amount: Amount.rvn(member.value, true),
      ...
      det.push({
        ...
        category: 'send',
        amount: -(Amount.rvn(member.value, true)),
        fee: -(Amount.rvn(details.fee, true)),
        vout: i
      });
    ...
    return {
      amount: Amount.rvn(receive ? received : -sent, true),
  ...
  async getUnconfirmedBalance(args, help) {
    ...
    return Amount.rvn(balance.unconfirmed, true);
  }
  ...
  async getWalletInfo(args, help) {
    ...
    return {
      walletid: wallet.id,
      walletversion: 6,
      balance: Amount.rvn(balance.unconfirmed, true),
      unconfirmed_balance: Amount.rvn(balance.unconfirmed, true),
      txcount: balance.tx,
      keypoololdest: 0,
      keypoolsize: 0,
      unlocked_until: wallet.master.until,
      paytxfee: Amount.rvn(this.wdb.feeRate, true)
    };
  }
  ...
  async importPrivKey(args, help) {
    if (help || args.length < 1 || args.length > 3) {
      throw new RPCError(errs.MISC_ERROR,
        'importprivkey "ravencoinprivkey" ( "label" rescan )');
    }
  ...
  async listAccounts(args, help) {
    ...
    for (const account of accounts) {
      ...
      map[account] = Amount.rvn(value, true);
    }
  ...
  async _listReceived(minconf, empty, watchOnly, account) {
    ...
    for (const entry of out) {
      ...
      entry.amount = Amount.rvn(entry.amount, true);
      result.push(entry);
    }
  ...
  async listUnspent(args, help) {
    ...
    for (const coin of coins) {
      ...
      out.push({
        ...
        amount: Amount.rvn(coin.value, true),
        confirmations: depth,
  ...
  async sendFrom(args, help) {
    if (help || args.length < 3 || args.length > 6) {
      throw new RPCError(errs.MISC_ERROR,
        'sendfrom "fromaccount" "toravencoinaddress"'
  ...
  async sendToAddress(args, help) {
    if (help || args.length < 2 || args.length > 5) {
      throw new RPCError(errs.MISC_ERROR,
        'sendtoaddress "ravencoinaddress" amount'
  ...
  async setAccount(args, help) {
    if (help || args.length < 1 || args.length > 2) {
      throw new RPCError(errs.MISC_ERROR,
        'setaccount "ravencoinaddress" "account"');
    }

    // Impossible to implement in rcoin:
    throw new Error('Not implemented.');
  }
  ...
  async signMessage(args, help) {
    if (help || args.length !== 2) {
      throw new RPCError(errs.MISC_ERROR,
        'signmessage "ravencoinaddress" "message"');
    }
