/*!
 * mtx.js - mutable transaction object for rcoin
...
class MTX extends TX {
   ...
   * @see ravencoin/src/wallet/wallet.cpp
   */
...
class FundingError extends Error {
  ...
  constructor(msg, available, required) {
    ...
    if (available != null) {
      this.message += ` (available=${Amount.rvn(available)},`;
      this.message += ` required=${Amount.rvn(required)})`;
