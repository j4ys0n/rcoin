/*!
 * tx.js - transaction object for rcoin
...
class TX {
  ...
  signatureHashV0(index, prev, type) {
    if ((type & 0x1f) === hashType.SINGLE) {
      // Ravencoind used to return 1 as an error code:
  ...
  format(view, entry, index) {
    ...
    return {
      ...
      value: Amount.rvn(this.getOutputValue()),
      fee: Amount.rvn(fee),
      rate: Amount.rvn(rate),
      minFee: Amount.rvn(this.getMinFee()),
      height: height,
      block: block,
      time: time,
      date: date,
   ...
   * will be reversed to abide by ravencoind's legacy
