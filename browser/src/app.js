...
function addItem(item, entry) {
  ...
  chainState.innerHTML = ''
    ...
    + `value=${Amount.rvn(node.chain.db.state.value)}`;
}
...
async function _formatWallet(wallet) {
  ...
  html += `Confirmed Balance: <b>${Amount.rvn(balance.confirmed)}</b><br>`;
  html += `Unconfirmed Balance: <b>${Amount.rvn(balance.unconfirmed)}</b><br>`;
