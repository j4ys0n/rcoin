...
const rcoin = require('../..');
const client = require('rclient');
const plugin = rcoin.wallet.plugin;
const network = rcoin.Network.get('regtest');

const node = new rcoin.FullNode({
  network: 'regtest',
  apiKey: 'foo',
  walletAuth: true,
  db: 'memory'
...
async function fundWallet(wdb, addr) {
  ...
  mtx.addOutpoint(new rcoin.Outpoint());
