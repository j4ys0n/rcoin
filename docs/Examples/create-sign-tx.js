...
(async () => {
  const master = rcoin.hd.generate();
  const key = master.derivePath('m/44/0/0/0/0');
  const keyring = new rcoin.wallet.WalletKey(key.privateKey);
  const cb = new rcoin.MTX();

  cb.addInput({
    prevout: new rcoin.Outpoint(),
    script: new rcoin.Script(),
    sequence: 0xffffffff
  });
  ...
  const coin = rcoin.Coin.fromTX(cb, 0, -1);
  coins.push(coin);
