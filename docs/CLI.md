Rcoin ships with rcoin-cli as its default HTTP client for command line access.
...
``` bash
$ export RCOIN_API_KEY=hunter2
$ export RCOIN_NETWORK=main
$ export RCOIN_URI=http://localhost:8767
$ rcoin cli info
```

``` bash
$ rcoin cli info --api-key=hunter2 --uri=http://localhost
```

``` bash
$ echo 'api-key: hunter2' > ~/cli.conf
$ rcoin cli info --config=~/cli.conf
```

## Examples

``` bash
$ export RCOIN_API_KEY=your-api-key
...
$ rcoin wallet history

# Send a transaction
$ rcoin wallet send [address] 0.01

# View balance
$ rcoin wallet balance

# Derive new address
$ rcoin wallet address

# Create a new account
$ rcoin wallet account create foo

# Send from account
$ rcoin wallet send [address] 0.01 --account=foo
```

RPC examples:

``` bash
$ rcoin rpc getblockchaininfo
$ rcoin rpc getwalletinfo
$ rcoin rpc getpeerinfo
$ rcoin rpc getbalance
$ rcoin rpc listtransactions
$ rcoin rpc sendtoaddress [address] 0.01
```

## Commands

rcoin-cli commands are split into 3 categories: cli, rpc, and wallet.
...
Rcoin implements nearly all ravencoind calls along with some custom calls.
